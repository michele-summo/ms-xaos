...
#define nextchar()                                                             \
    (uih->playstring == NULL ? xio_getc(FD) : uih->playstring[uih->playpos++])
#define ungetchar(c)                                                           \
    (uih->playstring == NULL ? xio_ungetc(c, FD) : uih->playpos--)
#define endoffile()                                                            \
    (uih->playstring == NULL ? xio_feof(FD)                                    \
                             : uih->playstring[uih->playpos] == 0)
...
void uih_line(uih_context *c, dialogparam *d)
{
    ...
    if (c->playstring != NULL) {
        seterr(TR("Error", "line available only in animation replay"));
        return;
    }
...
void uih_morphline(uih_context *c, dialogparam *d)
{
    ...
    if (l == NULL) {
        seterr(TR("Error", "Morphing non existing line!"));
        return;
    }
...
{
    if (!c->play) {
        seterr(TR("Error", "linekey not available in this context!"));
        return;
    }
...
void uih_update_lines(uih_context *c)
{
    ...
    number_t
        mmul = /*(tl_lookup_timer (c->playc->timer) - c->playc->starttime) /
                  (number_t) (c->playc->frametime - c->playc->starttime); */
        ...
        if (l->morph) {
            ...
            switch (l->mposmode) {
                ...
                case 1:
                    ...
                    if (x > y)
                        x = y;
                    mx1 = (int)(c->image->width / 2 +
                                (l->mx1 - 0.5) * x / c->image->pixelwidth);
                    my1 = (int)(c->image->height / 2 +
                                (l->my1 - 0.5) * x / c->image->pixelheight);
                    mx2 = (int)(c->image->width / 2 +
                                (l->mx2 - 0.5) * x / c->image->pixelwidth);
                    my2 = (int)(c->image->height / 2 +
...
{
    if (c->playstring != NULL) {
        seterr(TR("Error", "clear_line available only in animation replay"));
        return;
    }
    uih_removeline(c, uih_findkey(c, c->playc->lines.currkey++));
}
...
{
    if (c->playstring != NULL) {
        seterr(TR("Error", "clear_lines available only in animation replay"));
        return;
    }
...
int uih_loadcatalog(uih_context *c, const char *name)
{
    ...
    if (f == XIO_FAILED) {
        ...
        uih_error(c, TR("Error", "Catalog file not found"));
        return 0;
    }
    firsttime = 0;
    if (catalog != NULL)
                     ...
                     xio_constpath filename, int animr)
{
    ...
    const char *s;
    if (uih->play) {
        uih_error(uih, TR("Error", "Replay is already active"));
        return 0;
    }
    if (f == XIO_FAILED) {
        uih_error(uih, TR("Error", "File open failed"));
        return 0;
    }
    p = (struct uih_playcontext *)calloc(1, sizeof(*p));
    if (p == NULL) {
        uih_error(uih, TR("Error", "Out of memory"));
        return 0;
    }
    if (animr) {
...
static int gettoken(struct uih_context *uih)
{
    ...
    if (endoffile()) {
        if (uih->playstring) {
            seterr(TR("Error", "Missing parameter"));
        } else
            seterr(TR("Error", "Unexpected end of file"));
        return 0;
    }
    if ((c = nextchar()) == '"') {
        while (c == '\r')
        ...
        do {
            ...
            i++;
            if (i >= 1024) {
                seterr(TR("Error", "Token is too long"));
                i = 0;
            }
        } while (c != '"');
    } else
...
void uih_play_formula(struct uih_context *uih, const char *fname)
{
    ...
    seterr(TR("Error", "Unknown formula type"));
}

void uih_playmorph(struct uih_context *uih, dialogparam *d)
{
    if (uih->playstring != NULL) {
        seterr(TR("Error", "morph available only in animation replay"));
        return;
    }
    if (d[2].number <= 0 || d[3].number <= 0) {
        seterr(TR("Error", "morphview: Invalid viewpoint"));
...
{
    if (uih->playstring != NULL) {
        seterr(TR("Error", "move available only in animation replay"));
        return;
    }
    uih->playc->source = uih->fcontext->s;
...
{
    if (uih->playstring != NULL) {
        seterr(TR("Error", "morphjulia available only in animation replay"));
        return;
    }
...
{
    if (uih->playstring != NULL) {
        seterr(TR("Error", "morphangle available only in animation replay"));
        return;
    }
...
void uih_playusleep(struct uih_context *uih, int time)
{
    parsenext = 0;
    if (uih->playstring != NULL) {
        seterr(TR("Error", "sleep available only in animation replay"));
        return;
    }
    uih->playc->frametime = time;
    if (time < tl_lookup_timer(
                   uih->playc->timer) /*&&((!uih->step)||(!uih->zoomactive)) */
    ) {
        ...
        if (!uih->playc->timerin) {
            ...
            printf(TR("Error", "Internal program error #12 %i\n"),
                   uih->playc->playframe);
    }
...
{
    uih_playusleep(uih, 500000 + 1000000 *
                                     (uih->nletters + uih->todisplayletters) /
                                     uih->letterspersec);
    uih->nletters = 0;
...
void uih_playwait(struct uih_context *uih)
{
    parsenext = 0;
    if (uih->playstring != NULL) {
        seterr(TR("Error", "wait available only in animation replay"));
        return;
    }
    if (!uih->incomplete && !uih->display && !uih->recalculatemode &&
...
void uih_playcalculate(struct uih_context *uih) { uih_newimage(uih); }

void uih_playzoom(struct uih_context *uih) { uih->zoomactive = 1; }

void uih_playunzoom(struct uih_context *uih) { uih->zoomactive = -1; }

void uih_playstop(struct uih_context *uih) { uih->zoomactive = 0; }

void uih_playmessage(struct uih_context *uih, char *name)
{
    char *message;
    if (catalog == NULL) {
        uih_text(uih, TR("Error", "No catalog file loaded"));
        return;
    }
    message = find_text(catalog, name);
    if (message == NULL) {
        uih_text(uih, TR("Error", "Message not found in catalog file"));
        return;
    }
...
void uih_playload(struct uih_context *uih, xio_path file)
{
    ...
    xio_pathdata tmp;
    if (uih->playstring != NULL) {
        seterr(TR("Error", "load available only in animation replay"));
        return;
    }
    if (uih->playc->level == MAXLEVEL) {
        seterr(TR("Error", "Include level overflow"));
        return;
    }
    ...
    if (f == XIO_FAILED) {
        seterr(TR("Error", "File not found"));
        return;
    }
...
static void uih_processcommand(struct uih_context *uih, int flags)
{
    ...
    if (error != NULL)
        seterr(error);
    if (!last) {
        seterr(TR("Error", "Too many parameters"));
    }
}
...
void uih_playupdate(struct uih_context *uih)
{
    ...
    while (uih->play && uih->playc->playframe && errstring == NULL) {
        ...
        if (errstring != NULL) {
            ...
            if (uih->play) {
                sprintf(errtext, TR("Message", "Replay disabled at line %i"),
                        uih->playc->line);
