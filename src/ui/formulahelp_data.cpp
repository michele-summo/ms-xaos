...
const struct formula_help_row formula_help_functions[] = {
    ...
    {NULL, NULL, NULL, "randomness: all but the seed optional, and a new field every iteration; what the kaleidoscope, its mode and the radius do is in the Values tab. The radius is read the way bailout is and the field reaches twice its root, so a bare call against a bailout of the same number draws bands"},
    {"rand", "a", "real(a) times a random number in [0, 1); depends on call order, so a redraw differs", NULL},
    {"randsc", "seed; [size=1+i]; [degradation=0.5+0.5i]; [kaleidoscope=1]; [mode=0]; [radius=4]", "coherent noise over the point: soft blobs, size wide and size high. Gradient noise, so the blobs do not line up on the lattice", NULL},
    {"randscq", "seed; [size=1+i]; [degradation=0.5+0.5i]; [kaleidoscope=1]; [mode=0]; [radius=4]", "the same field with no interpolation: a mosaic of square cells, each with its own level and a lean across it so that a colouring mode has something to read inside a cell", NULL},
    {"randscp", "seed; [size=1+i]; [degradation=0.5+0.5i]; [kaleidoscope=1]; [mode=0]; [radius=4]", "the same field cut into irregular polygons with straight edges, each leaning its own way", NULL},
    {"randsch", "seed; [size=1+i]; [degradation=0.5+0.5i]; [kaleidoscope=1]; [mode=0]; [radius=4]", "the same field cut into hexagons: a honeycomb, each cell leaning its own way", NULL},
    {"randsct", "seed; [size=1+i]; [degradation=0.5+0.5i]; [kaleidoscope=1]; [mode=0]; [radius=4]", "the same field cut into equilateral triangles, alternating in orientation, each leaning its own way", NULL},
...
const struct formula_help_row formula_help_values[] = {
    ...
    {NULL, NULL, NULL, "randsc family: how far the field reaches, read the way bailout is read -- as the square of a distance, the field reaching twice its root, so that a radius equal to the bailout puts half the field outside it and the iteration count has something to count"},
    {"4", "", "the default, reaching 4, which is where a bailout of 4 stands", NULL},
    {"smaller", "", "a gentler field, for perturbing an iteration that escapes on its own: 0.015 reaches about a quarter", NULL},
