...
const struct formula_help_row formula_help_functions[] = {
    ...
    {NULL, NULL, NULL, "figures standing at the position, the same in mandelbrot and julia mode as the noise is -- but fractals of their own rather than fields: the two Sierpinski figures carry the point to the parent of the hole it stands in, so the pass it leaves on is the level that cut it and writing one alone draws the figure; the snowflake has no parent to walk to and is drawn whole. radius is read the way bailout is, as the square of the distance, and the figure is inscribed in the shape a bailout of that number draws: the gasket in the triangle at -90 degrees, the carpet in the square, the snowflake with its points on the corners of the hexagon at 0"},
    ...
    {"sierpinskyc", "[radius=4]; [squares=3]", "the Sierpinski carpet, cut into squares by squares with the middle one thrown away; read as sierpinskyt is. 2 gives a gasket, 5 or 7 a lacier carpet", NULL},
    {"snowflake", "[radius=4]", "the Koch snowflake, drawn whole: a point in it is handed back where it stands and never leaves, one out of it leaves at once -- its levels add to the edge rather than cutting into the middle, so there is no parent to walk to", NULL},
