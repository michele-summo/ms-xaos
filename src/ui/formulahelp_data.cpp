...
const struct formula_help_row formula_help_functions[] = {
    ...
    {"snowflake", "[radius=4]", "the Koch snowflake, read from its middle out and banded by generation: a regular hexagon, six triangles of its own side standing on its six sides, twelve of a third that on their free edges, and so on. The ground it leaves in the corners of the hexagon is no part of the figure -- it never leaves, so it takes the inside colour and the incolouring modes work there", NULL},
