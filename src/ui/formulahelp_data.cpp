...
const struct formula_help_row formula_help_values[] = {
    ...
    {NULL, NULL, NULL, "randsc family: the skew, which turns a cell's value by where in the cell the point stands so that the colouring modes have something to read inside a cell -- the value is multiplied by 1 + skew*(offset from the middle), the argument saying which way and the modulus how far"},
    {"0", "", "the default: the factor is one, the value stays on the real axis and is the number it always was, to the bit", NULL},
    {"0.01 to 0.05", "", "the useful range: every colouring mode comes to life, at the cost of a tenth of a per cent of the picture where a cell is cut by the bailout falling inside it", NULL},
    {"larger", "", "more colour and more cutting, in proportion", NULL},
