...
const struct formula_help_row formula_help_functions[] = {
    ...
    {NULL, NULL, NULL, "figures standing at the position, the same in mandelbrot and julia mode as the noise is -- but fractals of their own rather than fields: each hands back nothing until the pass reaches the level of the construction that decided the point and a number no bailout can hold once it has, so the pass a point escapes on is that level and writing one alone draws the figure. radius is read the way bailout is, as the square of the distance, and the figure is inscribed in the shape a bailout of that number draws: the gasket in the triangle at -90 degrees, the carpet in the square, the snowflake with its points on the corners of the hexagon at 0"},
    {"sierpinskyt", "[radius=4]", "the Sierpinski gasket: a point leaves on the pass numbered by the cut that took it, and one on the gasket never leaves", NULL},
    ...
    {"snowflake", "[radius=4]", "the Koch snowflake, drawn whole: a point in it never leaves and a point out of it leaves at once, its levels adding to the edge rather than cutting into the middle", NULL},
