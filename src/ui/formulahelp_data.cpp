...
const struct formula_help_row formula_help_functions[] = {
    ...
    {NULL, NULL, NULL, "figures standing at the position, the same in mandelbrot and julia mode as the noise is: 1 where the point belongs to the figure most, tapering as the feature it stands in gets finer, 0 where the figure is not. radius is read the way bailout is, as the square of the distance, and the figure is inscribed in the shape a bailout of that number draws: the gasket in the triangle at -90 degrees, the carpet in the square, the snowflake with its six points on the corners of the hexagon at 0. A bailout polygon stands its sides that distance from the centre, so its corners are further out -- twice as far for a triangle. Written alone a figure draws one flat tone, its value never leaving the bailout: add it to z, as in z+sierpinskyt(), and the levels become the bands of the picture"},
