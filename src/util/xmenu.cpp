...
static void x_menu_insert(const menuitem *item, struct entry *iitem, int n)
{
    int i;
    int len;
    int hashpos;
...
void menu_add(const menuitem *item, int n) { x_menu_insert(item, NULL, n); }
...
const static void *menu_rfind(const void *(*function)(struct entry *item),
                              const char *root)
{
    struct entry *item = firstitem;
    const void *r;
...
void menu_activate(const menuitem *item, struct uih_context *c, dialogparam *d)
{
    if (c == NULL &&
        ...
        if (c != NULL && c->incalculation && !(item->flags & MENUFLAG_INCALC)) {
    ...
    switch (item->type) {
        ...
        case MENU_INT:
            ...
            break;
        case MENU_STRING:
            ((void (*)(struct uih_context *, const char *))item->function)(
        ...
        case MENU_CUSTOMDIALOG:
            if (!menu_havedialog(item, c)) {
                ...
                if (
                    !strcmp(item->shortname, "uimandelbrot") &&
...
int menu_enabled(const menuitem *item, struct uih_context *c)
{
    if (item->flags & (MENUFLAG_RADIO | MENUFLAG_CHECKBOX))
        switch (item->type) {
            ...
            case MENU_INT:
                return (((int (*)(struct uih_context *, int))item->control)(
                    c, item->iparam));
            case MENU_STRING:
                return (((int (*)(struct uih_context *, const char *))
                             item->control)(c, (const char *)item->pparam));
            default:
                                ...
                                int scheme, int mask, const char *root)
{
    ...
    if (item->flags & mask) {
        sprintf(errorstr,
                "function '%s' not available in this context (%i, %i)", c, mask,
                item->flags);
        return errorstr;
    }
            ...
            while (e1) {
                ...
                if (e1->item->type != MENU_SUBMENU &&
                    ...
                    if (menu_havedialog(e1->item, NULL)) {
                        const menudialog *d = menu_getdialog(NULL, e1->item);
